...
//Rare exceptions to indicate a bug in the _tideradar code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_tideradar = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_tideradar]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.tideradar.module"
